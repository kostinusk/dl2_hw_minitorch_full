...
class Sigmoid(Function):
    @staticmethod
    def forward(ctx: Context, t1: Tensor) -> Tensor:
        ctx.save_for_backward(sigm := t1.f.sigmoid_map(t1))
        return sigm

    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        (sigm,) = ctx.saved_values
        return sigm * (-sigm + 1.0) * grad_output
...
class Exp(Function):
    @staticmethod
    def forward(ctx: Context, t1: Tensor) -> Tensor:
        ctx.save_for_backward(exp := t1.f.exp_map(t1))
        return exp
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        orderr = [int(order[i]) for i in range(order.size)]
        ctx.save_for_backward(orderr)
        return a._new(a._tensor.permute(*orderr))

    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        (order,) = ctx.saved_values
        reverse_order = [0 for _ in order]

        for i, dim in enumerate(order):
            reverse_order[dim] = i

        return (grad_output._new(grad_output._tensor.permute(*reverse_order)), 0)
